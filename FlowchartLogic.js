...
const DEFAULT_FLOWCHART_POSITIONS = {
  ...
  "F3_FAC_ABOVE": { x: 220.6871337890625, y: 111.1002604965264 }, "F3_FAC_BELOW": { x: 406.35528564453125, y: 113.37683272361755 },
  "F3_EDU1": { x: 216.475341796875, y: 199.54791259765625 }, "F3_EDU2": { x: 405.512939453125, y: 200.98212432861328 },
  "F3_OUT1": { x: 584.0929565429688, y: 152.96768569946289 }, "F3_OUT2": { x: 581.5658569335938, y: 224.56816864013672 },
  "F3_OUT3": { x: 403.8282165527344, y: 281.8485565185547 }, "F3_OUT4": { x: 215.6329803466797, y: 283.7837677001953 },
...
function drawFlowBoxes() {
  ...
  /** Same padding as autoFitFlowBox (used when adjusting layout after auto-fit). */
  const FLOW_BOX_PAD_TOP = 44;
  const FLOW_BOX_PAD_BOTTOM = 30;
  const FLOW_BOX_PAD_RIGHT = 30;
  const FLOW_BOX_STACK_GAP = 20;
  const FLOW_1_TO_2_GAP = 18;

  /**
   * Align flows 2–4 to one column, enforce vertical gaps without clipping nodes,
   * and keep flow 1 from extending into the flows 2–4 column.
   */
  function finalizeFlowBoxLayout(f1, f2, f3, f4) {
    ...
    if (ordered.length >= 2) {
      ordered.sort((a, b) => a.tightMinY - b.tightMinY);
      for (let iter = 0; iter < 12; iter++) {
        let changed = false;
        for (let i = 1; i < ordered.length; i++) {
          const prev = ordered[i - 1];
          const cur = ordered[i];
          const prevBottomMin = prev.tightMaxY + FLOW_BOX_PAD_BOTTOM;
          const prevBottom = prev.y + prev.height;
          const maxPrevBottomAllowed = cur.y - FLOW_BOX_STACK_GAP;
          // Need a clear gap between the bottom of prev and the top of cur.
          if (prevBottom <= maxPrevBottomAllowed) continue;

          if (maxPrevBottomAllowed >= prevBottomMin - 0.5) {
            const newH = maxPrevBottomAllowed - prev.y;
            if (Math.abs(newH - prev.height) > 0.5) {
              prev.height = newH;
              changed = true;
            }
          } else {
            const newCurY = prevBottom + FLOW_BOX_STACK_GAP;
            if (newCurY > cur.y + 0.5) {
              cur.y = newCurY;
              const h = cur.tightMaxY + FLOW_BOX_PAD_BOTTOM - cur.y;
              cur.height = Math.max(h, 48);
              changed = true;
            }
          }
        }
...
const FLOW3_NODE_IDS_FOR_RESET = [
  "F3_FAC_ABOVE",
  "F3_FAC_BELOW",
  "F3_EDU1",
  "F3_EDU2",
...
/**
 * Clears saved coordinates for Flow 3 only, applies DEFAULT_FLOWCHART_POSITIONS in memory,
 * and re-renders. Use when an old saved layout keeps Flow 3 under the section title.
 * Then use Layout → Edit layout to fine-tune, and Save Layout to remember this browser’s layout.
 */
window.resetFlow3NodesToDefaults = function () {
  try {
    nodes.forEach(function (n) {
      if (!n || n.flow !== 3) return;
    ...
    if (obj && typeof obj === "object") {
      FLOW3_NODE_IDS_FOR_RESET.forEach(function (id) {
        delete obj[id];
      });
    ...
    console.log("✅ Flow 3 nodes reset to built-in defaults (saved F3 keys removed).");
  } catch (e) {
