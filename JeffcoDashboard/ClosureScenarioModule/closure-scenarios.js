...
(function () {
  ...
  const OD_STUDENTS_CSV_PATH = _data("OD_Students.csv");
  const MAP_EXPORT_CSV_PATH = _data("09 Map_Export.csv");
  const SCHOOL_DISTANCES_CSV_PATH = _data("06 SchooltoSchoolDistances.csv"); // used for grade spans
  const ARTICULATION_AREAS_GEOJSON_PATH = _data("08 ArticulationArea.geojson");
  const CACHE_BUST = "20260508_1";
  ...
  async function loadDecisionData() {
    if (typeof window.loadDecisionColumnMap === "function") {
      await window.loadDecisionColumnMap();
    }
    const res = await parseCsv(DECISION_CSV_PATH, {});
    const rows = (res && res.data) ? res.data : [];
    if (typeof window.applyDecisionColumnMapToRows === "function") {
      window.applyDecisionColumnMapToRows(rows);
    }
    ...
    rows.forEach((r) => {
      ...
      const pk = Math.max(
        0,
        parseNumberMaybe(
          (pick && pick(r, "enrollmentPK")) ?? r.CurrPKEnrollment ?? r.PKEnrollment ?? r["PKEnrollment"] ?? r["PK Enrollment"]
        ) ?? 0
      );
  ...
  function ensureMap() {
    ...
    map.on("load", () => {
      ...
      const CS_ARTICULATION_OUTLINE_WIDTH = 1.5;
      const CS_ARTICULATION_SELECTED_OUTLINE_WIDTH = 4;
      let csSelectedArticulationArea = null;
      const refreshCsArticulationSelectionOutline = () => {
        try {
          if (!map.getLayer("cs-articulation-outline")) return;
          const selected = (csSelectedArticulationArea || "").trim();
          if (!selected) {
            map.setPaintProperty("cs-articulation-outline", "line-width", CS_ARTICULATION_OUTLINE_WIDTH);
            return;
          }
          map.setPaintProperty("cs-articulation-outline", "line-width", [
            "case",
            ["==", ["get", "__aaName"], selected],
            CS_ARTICULATION_SELECTED_OUTLINE_WIDTH,
            CS_ARTICULATION_OUTLINE_WIDTH,
          ]);
        } catch (_) {}
      };
      ...
      map.addLayer({
        ...
        paint: {
          "line-color": "#000000",
          "line-opacity": 1,
          "line-width": CS_ARTICULATION_OUTLINE_WIDTH,
        },
      });
      ...
      popup.on("close", () => {
        pinned = false;
        csSelectedArticulationArea = null;
        refreshCsArticulationSelectionOutline();
      });
      ...
      map.on("click", "cs-articulation-fill", (e) => {
        ...
        if (!areaName) return;
        pinned = true;
        csSelectedArticulationArea = areaName;
        refreshCsArticulationSelectionOutline();
