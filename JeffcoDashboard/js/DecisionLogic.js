...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function loadArticulationFromMapExport() {
    return new Promise((resolve) => {
      ...
      Papa.parse((window.jeffcoDataUrl && window.jeffcoDataUrl("09 Map_Export.csv")) || "../../Facility Data/09 Map_Export.csv", {
        download: true,
        header: true,
        skipEmptyLines: true,
        complete: (results) => {
  ...
  /**
   * Distance to the nearest underutilized school with grade overlap.
   * Must stay in sync with FlowchartLogic.js getSchoolDistanceToWelcoming.
   */
  function getDistanceToWelcomingSchool(row) {
    if (!row) return null;
    ...
    const raw =
      row["DistanceUnderutilizedschools"] ??
      row["Distance Underutilized Schools"] ??
      row["Distance to Underutilized"];
  ...
  function evaluateSchool(row, t = self.thresholds) {
    ...
    const decisions = {
      ...
      dist: (() => {
        ...
        const d = getDistanceToWelcomingSchool(row);
      ...
      edu3_2: (() => {
        ...
        if (typeof window.below50PctlEaYesNo === "function") {
          const computed = window.below50PctlEaYesNo(row, self.schoolData);
  ...
  function downloadTableCsv(table, filenameStem, options) {
    options = options || {};
    if (!table) return;
    try {
      const headerCells = Array.from(table.querySelectorAll('thead th'));
      const headers = headerCells.length
        ? headerCells.map(th => (th.querySelector('.th-label') || th).textContent.trim())
        : [];
      const rowSelector = options.includeHiddenRows ? 'tbody tr' : 'tbody tr:not(.filter-hidden)';
      const bodyRows = Array.from(table.querySelectorAll(rowSelector));
      const rowsOut = bodyRows.map(tr =>
        Array.from(tr.querySelectorAll('td')).map(td => (td.textContent || '').trim())
      );
      const lines = [];
      if (headers.length) lines.push(headers.map(csvEscapeCell).join(','));
      rowsOut.forEach(r => lines.push(r.map(csvEscapeCell).join(',')));
      if (options.includeFooter) {
        const footCells = Array.from(table.querySelectorAll('tfoot th, tfoot td'));
        if (footCells.length) {
          lines.push(footCells.map(cell => (cell.textContent || '').trim()).map(csvEscapeCell).join(','));
        }
      }
      ...
      a.href = url;
      a.download = `${filenameStem}_${yyyy}-${mm}-${dd}.csv`;
      ...
      console.warn('⚠️ Unable to export table CSV:', e);
    }
  }

  window.exportDomTableToCsv = downloadTableCsv;
  ...
  function renderTable(data) {
    ...
    const rows = data.map((row, idx) => {
      ...
      const schoolNameRaw = row["Building Name"] || "";
      const schoolName = ((window.formatSchoolDisplayName || ((name) => String(name ?? '').trim()))(schoolNameRaw)).replace(/"/g, "&quot;");
    ...
    if (exportSummaryBtn) {
      exportSummaryBtn.onclick = function () {
        const table = summaryDiv.querySelector('table.data-table');
        downloadTableCsv(table, 'Strategic_Decision_Summary', { includeHiddenRows: true, includeFooter: true });
      };
    }
      ...
      <div class="decision-by-school-wrap">
        ...
        <div class="decision-by-school-scroll">
          <table class="data-table decision-by-school-table">
            ...
            <thead>
              <tr>
                ${buildDecisionBySchoolHeaderCell('School Name', 0)}
                ${buildDecisionBySchoolHeaderCell('School Type', 1)}
                ${buildDecisionBySchoolHeaderCell('Articulation Area', 2)}
                ${buildDecisionBySchoolHeaderCell('Strategy Group', 3)}
                ${buildDecisionBySchoolHeaderCell('Project Type', 4)}
              </tr>
            </thead>
            <tbody>${rows}</tbody>
          </table>
    ...
    if (exportBtn) {
      exportBtn.onclick = function () {
        ...
        downloadTableCsv(table, 'Decision_By_School');
      };
    }

    makeTablesSortable(resultsDiv);
  }
  ...
  function buildDecisionBySchoolHeaderCell(label, colIndex) {
    const title = String(label).replace(/"/g, '&quot;');
    return (
      '<th class="sortable-header filterable-header" data-column="' + colIndex + '" data-type="string" title="' + title + '">' +
      '<span class="th-inner"><span class="th-label">' + label + '</span></span>' +
      ...
      '<div class="col-resize-handle" data-col="' + colIndex + '" title="Drag to resize column"></div>' +
      '</th>'
    );
  }

  function injectDecisionBySchoolStickyStyles() {
    let style = document.getElementById('decision-by-school-sticky-styles');
    if (!style) {
    ...
    style.textContent = `
      ...
      .decision-by-school-table tbody td::selection {
        background: #e8edf2;
        color: inherit;
      }
      .decision-by-school-table tbody td::-moz-selection {
        background: #e8edf2;
        color: inherit;
      }
      ...
      .decision-by-school-table th {
        position: relative;
      }
      .decision-by-school-table .filterable-header .th-inner {
        display: block;
        width: 100%;
        min-width: 0;
        padding-right: 14px;
      ...
      .decision-by-school-table th.sortable-header {
        cursor: pointer;
      }
      .decision-by-school-table th.sortable-header:hover {
        background: #dbeafe;
      }
      .decision-by-school-table th.sortable-header::after {
        content: '';
        position: absolute;
        right: 3px;
        top: 50%;
      ...
      .decision-by-school-table th.sortable-header.sort-asc::after {
        border-bottom-color: #007cbf;
        border-top: none;
        opacity: 1;
      }
      .decision-by-school-table th.sortable-header.sort-desc::after {
  ...
  function makeTablesSortable(root) {
    const table = root && root.querySelector('table.decision-by-school-table');
    if (!table) return;
    ...
    table.querySelectorAll('th.sortable-header').forEach(header => {
        header.addEventListener('click', (e) => {
            ...
            if (e.target && e.target.closest('.filter-dropdown')) return;
            if (e.target && e.target.closest('.col-resize-handle')) return;
            const tbody = table.querySelector('tbody');
            if (!tbody) return;
            ...
            table.querySelectorAll('th.sortable-header').forEach(h => h.classList.remove('sort-asc', 'sort-desc'));
    ...
    return new Promise((resolve, reject) => {
      const loadMap =
        typeof window.loadDecisionColumnMap === "function"
          ...
          : Promise.resolve();
      loadMap.then(() => {
      Papa.parse((window.jeffcoDataUrl && window.jeffcoDataUrl("01 Decision Data Export.csv")) || "../../Facility Data/01 Decision Data Export.csv", {
        download: true,
        header: true,
        skipEmptyLines: true,
        complete: (results) => {
          ...
          if (typeof window.applyDecisionColumnMapToRows === "function") {
            window.applyDecisionColumnMapToRows(results.data);
            window.applyDecisionColumnMapToRows(self.schoolData);
          }
          if (typeof window.normalizeDecisionExportRows === "function") {
            const eaMedian = window.normalizeDecisionExportRows(self.schoolData, results.data);
          } else if (typeof window.enrichDecisionRowsWithBelow50PctlEa === "function") {
            const eaMedian = window.enrichDecisionRowsWithBelow50PctlEa(self.schoolData, results.data);
            console.log(`📊 Below50PCTL_EA_Cat computed from EducationalAdequacy (median=${eaMedian}, cohort=all export rows)`);
          }
