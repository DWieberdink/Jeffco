...
const flowDisplaySchoolName = window.formatSchoolDisplayName || ((name) => String(name ?? '').trim());
...
// Bump when default Flow 3/4 coordinates change so stale saved layouts are cleared for those nodes.
const FLOWCHART_LAYOUT_VERSION = 5;
const FLOWCHART_F34_NODE_IDS = [
...
function migrateFlowchartLayoutVersion() {
  try {
    const ver = parseInt(localStorage.getItem("flowchartLayoutVersion") || "0", 10);
    if (ver >= FLOWCHART_LAYOUT_VERSION) return;
    const raw = localStorage.getItem("flowchartNodePositions") || "{}";
    const obj = JSON.parse(raw);
    if (obj && typeof obj === "object") {
      FLOWCHART_F34_NODE_IDS.forEach((id) => { delete obj[id]; });
      localStorage.setItem("flowchartNodePositions", JSON.stringify(obj));
    }
    localStorage.removeItem("flowBoxPositions");
    localStorage.setItem("flowchartLayoutVersion", String(FLOWCHART_LAYOUT_VERSION));
    console.log("📐 Flowchart layout v" + FLOWCHART_LAYOUT_VERSION + ": reset Flow 3–4 nodes and flow box overlays.");
  } catch (e) {
    console.warn("flowchart layout migration failed", e);
  }
}

// Canonical node centers (fx/fy). initializeFlowchartData uses this whenever localStorage has no override for that id.
// To ship a new default: update this object (or Save Layout → Export for GitHub → paste here).
const DEFAULT_FLOWCHART_POSITIONS = {
  ...
  "F3_FAC_ABOVE": { x: 216.07432556152344, y: 30.212926864624023 },
  "F3_FAC_BELOW": { x: 407.77392578125, y: 32.70253372192383 },
  "F3_EDU1": { x: 218.5639190673828, y: 123.57312774658203 },
  "F3_EDU2": { x: 407.77392578125, y: 122.32833099365234 },
  "F3_OUT1": { x: 589.51513671875, y: 36.43693923950195 },
  "F3_OUT2": { x: 592.0047607421875, y: 128.55233764648438 },
  "F3_OUT3": { x: 400.30511474609375, y: 206.97491455078125 },
  "F3_OUT4": { x: 222.29833984375, y: 206.97491455078125 },
  "F4_EDU1": { x: 204.87109375, y: 336.43438720703125 },
  "F4_FAC1": { x: 395.3258972167969, y: 342.6584167480469 },
  "F4_OUT1": { x: 584.5359497070312, y: 338.92401123046875 },
  "F4_FAC2": { x: 392.8363037109375, y: 434.7738037109375 },
  "F4_OUT2": { x: 587.0255126953125, y: 419.836181640625 },
  "F4_DIST": { x: 392.8363037109375, y: 523.15478515625 },
  "F4_OUT3": { x: 198.64707946777344, y: 519.42041015625 },
  "F4_OUT4": { x: 588.2703247070312, y: 518.1755981445312 }
};

async function applyElkLayoutIfAvailable() {
  migrateFlowchartLayoutVersion();
...
window.initializeFlowchartFromScript = function(svgElement) {
  ...
  g.append("g").attr("class", "flow-labels-layer");
...
/** Split "FLOW N – Title" into two centered lines (en dash, em dash, or hyphen). */
function flowBoxTitleLines(label) {
  if (!label) return [""];
...
function updateFlowLabelGeometryFromSelection(textSelection) {
  textSelection.each(function () {
    const text = d3.select(this);
    const d = text.datum();
    if (!d) return;
    const lines = flowBoxTitleLines(d.label);
    const cx = d.x + d.width / 2;
    const startY = lines.length === 1 ? d.y + 24 : d.y + 15;
    text
    ...
    lines.forEach((line, i) => {
      text.append("tspan").attr("x", cx).attr("dy", i === 0 ? 0 : 15).text(line);
    });
  });
}

function updateFlowLabelGeometry(d) {
  const sel = d3.select(`.flow-label[data-id="${d.id}"]`);
  if (sel.empty()) return;
  sel.datum(d);
  updateFlowLabelGeometryFromSelection(sel);
}
...
function drawFlowBoxes() {
  ...
  const FLOW_BOX_PAD_TOP = 48;
  const FLOW_BOX_PAD_BOTTOM = 30;
  const FLOW_BOX_PAD_LEFT = 30;
  const FLOW_BOX_PAD_RIGHT = 30;
  const FLOW_1_TO_2_GAP = 18;
  ...
  function autoFitFlowBox(flowNum) {
    ...
    const padLeft = 30;
    const padRight = 30;
    const padBottom = FLOW_BOX_PAD_BOTTOM;
    const padTop = FLOW_BOX_PAD_TOP;

    const x = minX - padLeft;
    const y = minY - FLOW_BOX_PAD_TOP;
    const width = (maxX - minX) + FLOW_BOX_PAD_LEFT + FLOW_BOX_PAD_RIGHT;
    const height = (maxY - minY) + FLOW_BOX_PAD_TOP + FLOW_BOX_PAD_BOTTOM;
    return {
      x,
      y,
      width,
  ...
  /**
   * Extend the bottom of the upper flow and the top of the lower flow so both meet at the
   * midpoint between their node clusters (Flow 2↔3 and Flow 3↔4).
   */
  function bridgeAdjacentFlowBoxes(prev, cur) {
    if (!prev || !cur) return;
    const midY = (prev.tightMaxY + cur.tightMinY) / 2;
    const minH = FLOW_BOX_PAD_TOP + FLOW_BOX_PAD_BOTTOM + 24;

    const newPrevH = midY - prev.y;
    if (newPrevH >= minH) prev.height = newPrevH;

    const curBottom = cur.tightMaxY + FLOW_BOX_PAD_BOTTOM;
    const newCurH = curBottom - midY;
    if (newCurH >= minH) {
      cur.y = midY;
      cur.height = newCurH;
    }
  ...
  /**
   * Align flows 2–4 to one column; snug-fit each box to its nodes, then bridge gaps at the midpoint.
   */
  function finalizeFlowBoxLayout(f1, f2, f3, f4) {
    const rightFits = [f2, f3, f4].filter(Boolean);
    if (rightFits.length) {
      let colLeft = Infinity;
      let colRight = -Infinity;
      rightFits.forEach((f) => {
        colLeft = Math.min(colLeft, f.x);
        colRight = Math.max(colRight, f.x + f.width);
      });
      rightFits.forEach((f) => {
        f.x = colLeft;
        f.width = colRight - colLeft;
      });
    }

    const ordered = [f2, f3, f4].filter(Boolean);
    ordered.forEach((f) => {
      f.y = f.tightMinY - FLOW_BOX_PAD_TOP;
      f.height = (f.tightMaxY - f.tightMinY) + FLOW_BOX_PAD_TOP + FLOW_BOX_PAD_BOTTOM;
    });

    if (f2 && f3) bridgeAdjacentFlowBoxes(f2, f3);
    if (f3 && f4) bridgeAdjacentFlowBoxes(f3, f4);

    if (f1 && f2) {
      const maxRight = f2.x - FLOW_1_TO_2_GAP;
      const minW = f1.tightMaxX + FLOW_BOX_PAD_RIGHT - f1.x;
      if (f1.x + f1.width > maxRight) {
        f1.width = Math.max(minW, maxRight - f1.x);
      }
    }
  }
  ...
  // Fills/strokes match index.html #decision-sliders .flow-section.flow1–flow4 (--flow-color: #007cbf, #1D4ED8, #f59e0b, #dc2626).
  flowBoxes = [
    {
      id: "flow1",
  ...
  if (flowBoxEditMode) {
    ...
    finalizeFlowBoxLayout(f1, f2, f3, f4);
    flowBoxes.forEach(b => {
      const fit =
        b.id === "flow1" ? f1 :
        b.id === "flow2" ? f2 :
  ...
  g.select(".flow-boxes")
    ...
    .attr("fill", d => d.color)
    .attr("stroke", d => d.strokeColor || "#64748b")
    .attr("stroke-width", 2)
    .attr("stroke-dasharray", "5,5")
    .attr("opacity", 0.3);
  
  // Flow titles: own layer above nodes so they are not covered; centered with halo for contrast.
  let labelLayer = g.select(".flow-labels-layer");
  if (labelLayer.empty()) {
    labelLayer = g.append("g").attr("class", "flow-labels-layer");
  }
  try {
    const root = g.node();
    const layerNode = labelLayer.node();
    if (root && layerNode && layerNode.parentNode === root) {
  ...
  labelLayer
    ...
    .style("opacity", 1)
    .each(function () {
      updateFlowLabelGeometryFromSelection(d3.select(this));
    });
}
...
function mapSliderKeyToThresholdKey(sliderId) {
  const mapping = {
    ...
    progFlow3Slider: "adequateProgramsMinFlow3",
...
// Get school's distance to welcoming schools (same resolver the map/tables use, so the
// flowchart, Decision by School and prioritization all agree).
function getSchoolDistanceToWelcoming(row) {
  if (!row) return null;
  if (typeof window !== "undefined" && typeof window.resolveDistanceToWelcomingSchool === "function") {
...
function getDistanceThresholdFromSliders(schoolLevelStr, buildingNameStr, useFlow4Sliders) {
  ...
  // Multi-Level often means K-8; infer from building name (e.g. "Bradford K-8 North")
...
function initializeFlowchartData() {
  ...
  // Positions: localStorage overrides per id; otherwise DEFAULT_FLOWCHART_POSITIONS (single source of truth).
  const getPos = (nodeId) => {
    if (savedPositions[nodeId]) {
      return { fx: savedPositions[nodeId].x, fy: savedPositions[nodeId].y };
    }
    const def = DEFAULT_FLOWCHART_POSITIONS[nodeId];
    if (def && Number.isFinite(def.x) && Number.isFinite(def.y)) {
      return { fx: def.x, fy: def.y };
    }
    return { fx: 0, fy: 0 };
  };
  ...
  const flowchartData = {
      nodes: [
        // FLOW 1 - MAIN DECISION TREE (Centered) — coords: DEFAULT_FLOWCHART_POSITIONS
        { id: "START", label: "START HERE", ...getPos("START"), type: "start" },
        
        // Flow 1 Decision Nodes (Vertical stack)
        { id: "F1_UTIL1", label: "Current enrollment or\nutilization below?", ...getPos("F1_UTIL1"), thresholdKey: "utilSlider", flow: 1 },
        { id: "F1_UTIL2", label: "Current\nutilization rate above?", ...getPos("F1_UTIL2"), thresholdKey: "utilHighSlider", flow: 1 },
        { id: "F1_GROWTH2", label: "Projected enrollment growth above?", ...getPos("F1_GROWTH2"), thresholdKey: "growthSlider", flow: 1 },
        { id: "F1_DIST", label: "Distance to\nwelcoming schools", ...getPos("F1_DIST"), thresholdKey: "F1_DIST_dynamic", flow: 1 },
        { id: "F1_GROWTH", label: "Projected enrollment growth above?", ...getPos("F1_GROWTH"), thresholdKey: "growthSlider", flow: 1 },
        
        // FLOW 2 - BUILDING ADDITION (Left column)
        { id: "F2_ATTENDANCE", label: "Attendance area enrollment above", ...getPos("F2_ATTENDANCE"), thresholdKey: "attendanceAreaEnrollmentSlider", flow: 2 },
        { id: "F2_EXPAND", label: "Property has\nspace to expand?", ...getPos("F2_EXPAND"), thresholdKey: "siteCapacitySlider", flow: 2 },
        { id: "F2_FAC", label: "Composite Building Score\nabove?", ...getPos("F2_FAC"), thresholdKey: "buildSlider", flow: 2 },
        { id: "F2_EDU1", label: "Educational Adequacy\nabove?", ...getPos("F2_EDU1"), thresholdKey: "progSlider", flow: 2 },
        { id: "F2_EDU2", label: "Educational Adequacy\nabove?", ...getPos("F2_EDU2"), thresholdKey: "progSlider", flow: 2 },
        { id: "F2_OUT1", label: "Building\nAddition", ...getPos("F2_OUT1"), type: "outcome", flow: 2 },
        { id: "F2_OUT2", label: "Policy Solution\nfor Overcrowding", ...getPos("F2_OUT2"), type: "outcome", flow: 2 },
        { id: "F2_OUT3", label: "Building Replacement", ...getPos("F2_OUT3"), type: "outcome", flow: 2 },
        { id: "F2_OUT4", label: "Building Addition\nwith Capital Investment", ...getPos("F2_OUT4"), type: "outcome", flow: 2 },
        
        // FLOW 3 - MAINTENANCE/INVESTMENT (Center column)
        { id: "F3_FAC_ABOVE", label: "Composite Building Score\nabove?", ...getPos("F3_FAC_ABOVE"), thresholdKey: "buildAboveSlider", flow: 3 },
        { id: "F3_FAC_BELOW", label: "Composite Building Score\nbelow?", ...getPos("F3_FAC_BELOW"), thresholdKey: "buildBelowSlider", flow: 3 },
        { id: "F3_EDU1", label: "Educational Adequacy\nabove?", ...getPos("F3_EDU1"), thresholdKey: "progFlow3Slider", flow: 3 },
        { id: "F3_EDU2", label: "Below 50% percentile EA or\nsafety/security issues?", ...getPos("F3_EDU2"), flow: 3 },
        { id: "F3_OUT1", label: "Targeted Capital\nInvestment", ...getPos("F3_OUT1"), type: "outcome", flow: 3 },
        { id: "F3_OUT2", label: "Standard\nMaintenance", ...getPos("F3_OUT2"), type: "outcome", flow: 3 },
        { id: "F3_OUT3", label: "Major Capital\nInvestment", ...getPos("F3_OUT3"), type: "outcome", flow: 3 },
        { id: "F3_OUT4", label: "Building Replacement", ...getPos("F3_OUT4"), type: "outcome", flow: 3 },
        
        // FLOW 4 - CONSOLIDATION/CLOSURE (Right column)
        { id: "F4_EDU1", label: "Educational Adequacy\nabove?", ...getPos("F4_EDU1"), thresholdKey: "progSlider", flow: 4 },
        { id: "F4_FAC1", label: "Composite Building Score\nabove?", ...getPos("F4_FAC1"), thresholdKey: "buildFlow4Slider", flow: 4 },
        { id: "F4_FAC2", label: "Composite Building Score\nabove?", ...getPos("F4_FAC2"), thresholdKey: "buildFlow4Slider", flow: 4 },
        { id: "F4_DIST", label: "Within distance threshold\n of Welcoming School", ...getPos("F4_DIST"), thresholdKey: "elementaryDistanceSlider", flow: 4 },
        { id: "F4_OUT1", label: "Welcoming\nSchool", ...getPos("F4_OUT1"), type: "outcome", flow: 4 },
        { id: "F4_OUT2", label: "Welcoming School with\nCapital Investment", ...getPos("F4_OUT2"), type: "outcome", flow: 4 },
        { id: "F4_OUT3", label: "Closure (Goes\n to Welcoming School)", ...getPos("F4_OUT3"), type: "outcome", flow: 4 },
        { id: "F4_OUT4", label: "Welcoming School\n with Building Replacement", ...getPos("F4_OUT4"), type: "outcome", flow: 4 },
      ],
      links: [
                        // FLOW 1 LINKS
        ...
        { source: "F3_EDU2", target: "F3_OUT1", label: "Yes" },
        { source: "F3_EDU2", target: "F3_OUT2", label: "No" },
...
function populateMainFlowchartDropdown() {
  try {
    ...
    names.forEach(name => {
      ...
      opt.value = name;
      opt.textContent = flowDisplaySchoolName(name);
...
function loadSchoolData() {
  ...
  if (window.decisionLogic && Array.isArray(window.decisionLogic.schoolData) && window.decisionLogic.schoolData.length > 0) {
    ...
    if (typeof document !== 'undefined') {
      ...
      const progF3 = document.getElementById("progFlow3Slider");
      if (progF3) window.thresholds.adequateProgramsMinFlow3 = parseInt(progF3.value, 10);
      const buildBelow = document.getElementById("buildBelowSlider");
      if (buildBelow) window.thresholds.buildingThresholdBelow = parseFloat(buildBelow.value);
    }
  ...
  const startFallbackCsvLoad = () => Papa.parse((window.jeffcoDataUrl && window.jeffcoDataUrl("01 Decision Data Export.csv")) || "../../Facility Data/01 Decision Data Export.csv", {
    download: true,
    header: true,
    skipEmptyLines: true,
    complete: function (res) {
      ...
      if (typeof window.applyDecisionColumnMapToRows === "function") {
        window.applyDecisionColumnMapToRows(res.data);
        window.applyDecisionColumnMapToRows(schoolData);
      }
      if (typeof window.normalizeDecisionExportRows === "function") {
        window.normalizeDecisionExportRows(schoolData, res.data);
      } else if (typeof window.enrichDecisionRowsWithBelow50PctlEa === "function") {
        window.enrichDecisionRowsWithBelow50PctlEa(schoolData, res.data);
      }
      ...
      if (typeof document !== 'undefined') {
        ...
        if (utilHighSlider) window.thresholds.utilizationHigh = parseFloat(utilHighSlider.value) / 100;
        if (growthSlider) window.thresholds.enrollmentGrowth = parseFloat(growthSlider.value) / 100;
        const progF3b = document.getElementById("progFlow3Slider");
        if (progF3b) window.thresholds.adequateProgramsMinFlow3 = parseInt(progF3b.value, 10);
        const buildBelowB = document.getElementById("buildBelowSlider");
        if (buildBelowB) window.thresholds.buildingThresholdBelow = parseFloat(buildBelowB.value);
      }
  ...
  const mapReady =
    typeof window.loadDecisionColumnMap === "function"
      ...
      : Promise.resolve();
  mapReady.then(startFallbackCsvLoad);
...
function getEffectiveEnrollmentFlow(row) {
  ...
  const pkRaw =
    ...
    row["PK Enrollment"] ??
    "";
  const e = parseFloat(String(eRaw).replace(/,/g, "").trim()) || 0;
  const pk = parseFloat(String(pkRaw).replace(/,/g, "").trim()) || 0;
...
function evaluatePath(row, t) {
  ...
  const decisions = {
    ...
    dist: (() => {
      ...
      const d = getSchoolDistanceToWelcoming(row);
      return Number.isFinite(d) && d <= distanceThreshold ? "Yes" : "No";
    })(),
    growth: (() => {
    ...
    edu3_2: (() => {
      const cat = row["Below50PCTL_EA_Cat"];
      if (cat === "Yes" || cat === "yes" || cat === "YES") return "Yes";
      if (typeof window.below50PctlEaYesNo === "function") {
        const cohort = (window.decisionLogic && window.decisionLogic.schoolData) || schoolData;
        const computed = window.below50PctlEaYesNo(row, cohort);
        return computed === "Yes" ? "Yes" : "No";
      }
      return "No";
    })(),
    // Node label: "Composite Building Score above?"
  ...
  if (
    ...
    // Growth is what sent this school to consolidation, so keep F1_GROWTH on the path:
    // otherwise the deciding node renders greyed out and the highlight jumps from
    // F1_DIST straight into Flow 4 with no connecting link.
    if (!path.includes("F1_GROWTH")) path.push("F1_GROWTH");
  }
    ...
    if (decisions.fac3_above === "Yes") {
      ...
      if (decisions.fac3_below === "Yes") {
        ...
        if (decisions.edu3_2 === "Yes") {
          path.push("F3_OUT1"); // TARGETED CAPITAL INVESTMENT (below 50% EA or safety/security issues)
        } else {
...
function highlightFlow(path, decisions, currentFlow) {
  ...
  d3.selectAll(".flow-label").transition().duration(400).style("opacity", 1);
...
function resetFlowchartVisibility() {
  ...
  d3.selectAll(".flow-label").transition().duration(400).style("opacity", 1); // Titles stay readable
}
...
/**
 * Clears saved coordinates for Flow 3–4, applies DEFAULT_FLOWCHART_POSITIONS in memory,
 * and re-renders. Use when an old saved layout overlaps section titles or stacks flows too low.
 */
window.resetFlow3NodesToDefaults = function () {
  try {
    nodes.forEach(function (n) {
      if (!n || (n.flow !== 3 && n.flow !== 4)) return;
      const p = DEFAULT_FLOWCHART_POSITIONS[n.id];
    ...
    const obj = JSON.parse(raw);
    if (obj && typeof obj === "object") {
      FLOWCHART_F34_NODE_IDS.forEach(function (id) {
        delete obj[id];
      });
      localStorage.setItem("flowchartNodePositions", JSON.stringify(obj));
    }
    ...
    if (typeof renderFlowchart === "function") renderFlowchart();
    console.log("✅ Flow 3–4 nodes reset to built-in defaults.");
  } catch (e) {
    console.error("resetFlow3NodesToDefaults failed:", e);
  }
};
...
function loadMapExportData(callback) {
  if (mapExportData) { callback && callback(); return; }
      Papa.parse("https://raw.githubusercontent.com/DWieberdink/Jeffco/main/Data%20Source/09%20Map_Export.csv", {
    download: true,
    header: true,
    skipEmptyLines: true,
    complete: function(res) {
...
function updateFlowchartSchoolInfo(name) {
  ...
  const totalRaw =
    (pick && pick(row, "enrollmentTotal")) ??
    getVal(row, ["Enrollment", "Enrollment2026", "Enrollment2025", "Enrollment"]);
  const pkRawPick =
    (pick && pick(row, "enrollmentPK")) ??
    getVal(row, ["CurrPKEnrollment", "PKEnrollment", "PKEnrollment2026", "PKEnrollment2025", "PK Enrollment"]);
  const totalEnr = parseFloat((totalRaw || "").toString().replace(/,/g, "").trim()) || 0;
  const pkEnr = parseFloat((pkRawPick || "").toString().replace(/,/g, "").trim()) || 0;
  ...
  // Below 50th percentile EA — computed from EducationalAdequacy vs district median
  let below50PercentileEA = getVal(row, ["Below50PCTL_EA_Cat"]);
  if (below50PercentileEA === undefined || below50PercentileEA === null || below50PercentileEA === "") {
    if (typeof window.below50PctlEaYesNo === "function") {
      const cohort = (window.decisionLogic && window.decisionLogic.schoolData) || schoolData;
      below50PercentileEA = window.below50PctlEaYesNo(row, cohort);
    }
  }
  if (below50PercentileEA === undefined || below50PercentileEA === null || below50PercentileEA === "") {
  ...
  const displayName = flowDisplaySchoolName(name);
  ...
  <div style='font-family:"Franklin Gothic Book", "Franklin Gothic", "Arial Narrow", Arial, sans-serif; font-size:11px; margin-bottom:1px; line-height:1.3; display:flex; flex-wrap:wrap; gap:8px; align-items:center;'>
    ...
    <span>${(window.yearLabels && window.yearLabels.futureGrowthFlowchart) ? window.yearLabels.futureGrowthFlowchart() : 'Future Enrollment Growth (2030)'}: <strong>${growth}</strong></span>
  </div>
  ...
  <div style='font-family:"Franklin Gothic Book", "Franklin Gothic", "Arial Narrow", Arial, sans-serif; font-size:11px; margin-bottom:1px; line-height:1.3; display:flex; flex-wrap:wrap; gap:8px; align-items:center;'>
    ...
    <span>Composite Building Score: <strong>${buildingScore}</strong></span>
...
function showNearbyWelcomingSchools(originName) {
  ...
  finalCandidates.forEach(c => {
    ...
    html += "<tr>" +
      `<td style='border:1px solid #ccc;padding:2px 4px;'>${flowDisplaySchoolName(c.name)}</td>` +
