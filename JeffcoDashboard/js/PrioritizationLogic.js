...
window.prioritizationLogic = {
  ...
  /** Shared with School Profile Custom view so Priority Score tracks Step 4 sliders. */
  WEIGHTS_STORAGE_KEY: "jeffco_prioritization_weights_v1",

  /**
   * Group scope drives the normalization cohort, so School Profile must use the
   * same selection as Step 4 or identical schools score differently.
   */
  persistedStrategyScope: ["__ALL__"],

  persistWeights: function () {
    try {
      if (typeof localStorage === "undefined") return;
      localStorage.setItem(
        this.WEIGHTS_STORAGE_KEY,
        JSON.stringify({
          currentWeights: this.currentWeights,
          enabledWeights: this.enabledWeights,
          strategyScope: this.persistedStrategyScope,
        })
      );
      try {
        if (!this._scoreBroadcast) {
          this._scoreBroadcast = new BroadcastChannel("jeffco_priority_scores");
        }
        this._scoreBroadcast.postMessage({ type: "weights", updatedAt: Date.now() });
      } catch (_) {
  ...
  persistStrategyScope: function (groupNames) {
    const names = Array.isArray(groupNames) ? groupNames.filter(Boolean) : [];
    this.persistedStrategyScope = names.length ? names : ["__ALL__"];
    this.persistWeights();
  },

  /** Expand the Step 4 combined selectors into concrete strategy group names. */
  resolveStrategyScope: function (groupNames) {
    const names =
      Array.isArray(groupNames) && groupNames.length
        ? groupNames
        : this.persistedStrategyScope || ["__ALL__"];
    if (names.includes("__ALL__")) {
      return ["Expansion", "Maintenance/Investment", "Closure/Consolidation"];
    }
    if (names.includes("__ALL_EXP_MAINT__")) {
      return ["Expansion", "Maintenance/Investment"];
    }
    return names.filter((n) => this.strategyGroups[n]);
  },

  /**
   * Step 4 scores depend on map-derived inputs (nearest underutilized school) and the
   * live decision cohort, which the School Profile popout cannot reproduce exactly.
   * Publishing the ranked results makes that page mirror the dashboard instead of
   * approximating it.
   */
  SCORES_STORAGE_KEY: "jeffco_priority_scores_v1",

  publishScores: function (rankedSchools) {
    try {
      if (typeof localStorage === "undefined") return;
      if (!Array.isArray(rankedSchools) || !rankedSchools.length) return;
      const scores = [];
      rankedSchools.forEach((s) => {
        if (!s) return;
        const name = s["Building Name"] || s.name || s.SchoolName || s["School Name"] || s.School;
        const score = Number(s.priorityScore);
        if (!name || !Number.isFinite(score)) return;
        const uid = String(s.UniqueID || s["UniqueID"] || "").trim();
        scores.push({
          name: String(name),
          uid,
          group: s.strategyGroup || "",
          score,
        });
      });
      if (!scores.length) return;
      const body = JSON.stringify({
        weightsSig: localStorage.getItem(this.WEIGHTS_STORAGE_KEY) || "",
        scope: this.resolveStrategyScope(),
        updatedAt: Date.now(),
        scores,
      });
      if (body === this._lastPublishedScoresBody) return;
      this._lastPublishedScoresBody = body;
      localStorage.setItem(this.SCORES_STORAGE_KEY, body);
      // Same-origin popouts hear storage events; BroadcastChannel covers more cases
      // (and same-tab listeners if any) so every school row can refresh together.
      try {
        if (!this._scoreBroadcast) {
          this._scoreBroadcast = new BroadcastChannel("jeffco_priority_scores");
        }
        this._scoreBroadcast.postMessage({ type: "scores", updatedAt: Date.now() });
      } catch (_) {
        /* BroadcastChannel unavailable */
      }
    } catch (e) {
  ...
  restoreWeights: function () {
    try {
      ...
      const raw = localStorage.getItem(this.WEIGHTS_STORAGE_KEY);
      if (!raw) return false;
      const parsed = JSON.parse(raw);
      if (!parsed || typeof parsed !== "object") return false;
      Object.keys(this.strategyGroups).forEach((groupName) => {
        const savedW = parsed.currentWeights && parsed.currentWeights[groupName];
        if (savedW && typeof savedW === "object") {
          this.currentWeights[groupName] = {
            ...this.defaultWeights[groupName],
            ...savedW,
          };
        }
        const savedE = parsed.enabledWeights && parsed.enabledWeights[groupName];
        if (savedE && typeof savedE === "object") {
          const base = this.enabledWeights[groupName] || {};
          this.enabledWeights[groupName] = { ...base, ...savedE };
        }
      });
      if (Array.isArray(parsed.strategyScope) && parsed.strategyScope.length) {
        this.persistedStrategyScope = parsed.strategyScope;
      }
      return true;
    } catch (e) {
      return false;
  ...
  initialize: function(schoolDataWithDecisions) {
    ...
    // Restore Step 4 slider weights from the last dashboard session (if any).
    this.restoreWeights();
  ...
  // Distance to nearest underutilized overlapping school (mi). Prefer school-to-school matrix; fall back to export columns.
  // Returns null when no eligible school exists (display N/A, score 100).
  getDistanceFromOtherSchools: function (school) {
    if (!school) return null;
    // Shared resolver keeps this identical to the flowchart and Decision by School table.
    if (typeof window !== "undefined" && typeof window.resolveDistanceToWelcomingSchool === "function") {
      const d = window.resolveDistanceToWelcomingSchool(school);
      return Number.isFinite(d) ? d : null;
    }
    if (typeof window !== "undefined" && typeof window.getNearestUnderutilizedOverlappingSchool === "function") {
      const nearest = window.getNearestUnderutilizedOverlappingSchool(school);
      if (nearest && Number.isFinite(nearest.distanceMiles)) {
        return nearest.distanceMiles;
      }
      return null;
    }
    const keys = [
      ...
      "DistanceToUnderutilizedSchools",
    ];
    for (let i = 0; i < keys.length; i++) {
      const raw = school[keys[i]];
    ...
    switch (metricKey) {
      case "utilizationRate": {
        ...
        if (typeof window !== 'undefined' && window.getEffectiveUtilization) {
          ...
          const pick =
            typeof window !== "undefined" && typeof window.pickDecisionRowField === "function"
              ? window.pickDecisionRowField
              : null;
          ...
          const enr = parseFloat((pick && pick(school, "enrollmentTotal")) ?? school.Enrollment2026 ?? school.Enrollment2025 ?? school['Enrollment2025'] ?? school.Enrollment ?? 0) || 0;
          const pk = parseFloat((pick && pick(school, "enrollmentPK")) ?? school.PKEnrollment ?? school['PK Enrollment'] ?? 0) || 0;
      ...
      case "enrollment":
        return parseInt(
          (typeof window !== "undefined" && typeof window.pickDecisionRowField === "function"
            ? window.pickDecisionRowField(school, "enrollmentTotal")
            : undefined) ??
            school.Enrollment ??
            school.Enrollment2026 ??
            school.Enrollment2025 ??
            school["Enrollment2025"] ??
            0,
          10
        );
      ...
      case "distanceFromOtherSchools":
        return this.getDistanceFromOtherSchools(school);
  ...
  calculatePriorityScore: function(school, strategyGroupName, normalizationStats) {
    ...
    // Distance from other schools — greater distance = higher priority.
    // No eligible underutilized school → N/A value, fixed score 100.
    const distance = this.getDistanceFromOtherSchools(school);
    let distanceScore;
    if (distance === null || distance === undefined) {
      distanceScore = 100;
    } else {
      const distanceStat = getStat("distanceFromOtherSchools");
      distanceScore = this.normalizeValue(distance, distanceStat.min, distanceStat.max, false);
    }
  ...
  resetWeightsToDefaults: function() {
    Object.keys(this.strategyGroups).forEach(groupName => {
      this.currentWeights[groupName] = { ...this.defaultWeights[groupName] };
      const keys = Object.keys(this.defaultWeights[groupName] || {});
      const enabledMap = {};
