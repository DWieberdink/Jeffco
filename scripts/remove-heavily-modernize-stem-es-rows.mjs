...
const FALLBACK_PATH = path.join(__dirname, "..", "JeffCoProjectListAllSchools.no-stem-es-labs.csv");
  ...
  if (err && (err.code === "EBUSY" || err.code === "EPERM")) {
    ...
    console.warn(
      `Could not overwrite ${CSV_PATH} (file is open elsewhere). Wrote filtered copy to:\n  ${FALLBACK_PATH}\n` +
        `Close Excel/VS Code preview, delete or rename the original CSV, then rename this file to JeffCoProjectListAllSchools.csv`
    );
